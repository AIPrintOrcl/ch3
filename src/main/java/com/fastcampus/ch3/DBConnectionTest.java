...
public class DBConnectionTest {
    public static void main(String[] args) throws Exception {
        ...
        Statement stmt  = conn.createStatement(); // Statement를 생성한다. 연결된 데이터베이스에 명령을 내리려면 Statement가 필요하다.

        String query = "SELECT now()"; // 시스템의 현재 날짜시간을 출력하는 쿼리(query)
        ResultSet rs = stmt.executeQuery(query); // query를 실행한 결과를 rs에 담는다. ResultSet : 표 형식으로 데이터를 담는다.
